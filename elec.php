                ...
                <main>
                    <div class="container-fluid">
						...
						<li class="breadcrumb-item"><a href="index.php">Overall Status</a></li>
							...
							<li class="breadcrumb-item"><a href="mech.php">Electrical-Process</a></li>
                       ...
                       <div class="card mb-4">
                            <div class="card-header"><i class="fas fa-table mr-1"></i>Sectional Activites Status Table
                            ...
                            <div class="card-body">
                                <div class="table-responsive table-hover">
                                     <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
										 <?php $query=$conn->query("SELECT * FROM `main_act` WHERE `section`='elec-p'");
											if (!$query) die($conn->error);
											$i=1;
											while($row = $query->fetch_assoc())
											{
												...
												echo "<td>".$i."</td>";
												$i++;
												echo"<td> <a href=\"elec-act.php?act=".$row['eno']."\">".$row['act_name']."</td>";
...
var logchart = new Chart(ctx, {
  ...
  data: {
    labels: [			
			...
			<?php $query=$conn->query("SELECT * FROM `sec_act_log` WHERE `section`='elec-p'");
                                    if (!$query) die($conn->error);
									while($row=$query->fetch_assoc()){
    ...
    datasets: [{
      label: "Completion in %",
      data: [<?php $query=$conn->query("SELECT * FROM `sec_act_log` WHERE `section`='elec-p'");
                                    if (!$query) die($conn->error);
									while($row=$query->fetch_assoc()){
										echo $row['qty'].", ";
									}?>],

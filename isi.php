            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">In-Service Inspection Status</h1>
                        <ol class="breadcrumb mb-4">
						<li class="breadcrumb-item"><a href="index.php">Overall Status</a></li>
                            <?php if(isset($_GET['act'])){?>
							<li class="breadcrumb-item"><a href="isi.php">In-Service Inspection</a></li>
							...
							else echo "<li class='breadcrumb-item active'>In-Service Inspection</a></li>";
							?>
                        </ol>
                            ...
                            <div class="card-body">
                                <div class="table-responsive table-hover">
                                     <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
										 <?php $query=$conn->query("SELECT * FROM `main_act` WHERE `section`='isi'");
											if (!$query) die($conn->error);
											$i=1;
											while($row = $query->fetch_assoc())
											{
												...
												echo "<td>".$i."</td>";
												$i++;
												echo"<td> <a href=\"isi-act.php?act=".$row['eno']."\">".$row['act_name']."</td>";
...
var logchart = new Chart(ctx, {
  ...
  data: {
    labels: [			
			...
			<?php $query=$conn->query("SELECT * FROM `sec_act_log` WHERE `section`='isi'");
                                    if (!$query) die($conn->error);
									while($row=$query->fetch_assoc()){
    ...
    datasets: [{
      label: "Completion in %",
      data: [<?php $query=$conn->query("SELECT * FROM `sec_act_log` WHERE `section`='isi'");
                                    if (!$query) die($conn->error);
									while($row=$query->fetch_assoc()){
										echo $row['qty'].", ";
									}?>],

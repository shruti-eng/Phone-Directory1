            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">Instrumentation-Process Status</h1>
                        <ol class="breadcrumb mb-4">
						<li class="breadcrumb-item"><a href="index.php">Overall Status</a></li>
                            <?php if(isset($_GET['act'])){?>
							<li class="breadcrumb-item"><a href="inst.php">Instrumentation-Process</a></li>
							...
							else echo "<li class='breadcrumb-item active'>Instrumentation-Process</a></li>";
							?>
                        </ol>
                        <div class="card mb-4">
                            <div class="card-header"><i class="fas fa-table mr-1"></i>Activity Status Table 
							<div class="btn-group" style="float:right;">
							<?php if(isset($_SESSION['section'])){if($_SESSION['section']=='inst-p'){?><button style="float:right;" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#form-modal">Update</button><?php }}?>
							<button class="btn btn-outline-secondary btn-sm" data-toggle="collapse" href="#logchartdiv" role="button" aria-expanded="false" aria-controls="collapseExample" style="float:right;">
							Show Progress
              				</button>
							<button class="btn btn-info" onclick="window.open('print.php?act=<?php echo $_GET['act']  ?>','test');">Print</button>
            			</div>
                                        ...
                                        <tbody>
										 <?php $query=$conn->query("SELECT * FROM `sub_act` WHERE `main_act`='".$_GET['act']."'");
											...
											while($row = $query->fetch_assoc())
											{
												...
												echo"<td>".$row['qty_exec']."&nbsp&nbsp<i class=\"fa fa-chart-line\" aria-hidden=\"true\" onclick=\"progress_show('".$row['eno']."')\" style=\"cursor:pointer;\"></i></a>
												</td>";
												echo "</tr>";
											}?>
...
<br>
    ...
    <div class="modal fade" id="testdiv" tabIndex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="dialog">
		<div class="modal-content" style="float:right;">
		<div class="modal-header"><h4 class="modal-title" id="myModalLabel2">Data Saving Failed</h4>
		</div>
		<div class="modal-footer">
		<button class="btn btn-primary" id="close3" data-dismiss="modal">Close</button>
		</div>
		</div>
		</div>
		</div>
...
var myPieChart = new Chart(ctx, {
  type: 'pie',
  data: {
    labels: ["inst-p", "inst-p", "Inst-P", "ISI", "Prod-P"],
    datasets: [{data: [ 
...
$("#save").click(function(){
	$.ajax({
        type:"GET",
        url: 'save.php?act=<?php echo $_GET['act'];?>&sec=inst-p',
